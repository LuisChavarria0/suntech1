...
const LABELS: Record<
  Locale,
  {
    ...
    headerNote: string;
    description: (quote: QuoteResult) => string;
  }
> = {
  es: {
    ...
    description: (q) =>
      `Sistema Solar Fotovoltaico (SFV) con inyección a la red eléctrica, diseñado para un consumo promedio de ${q.consumoMensualKwh} kWh mensuales. El sistema está compuesto por ${q.panelesNecesarios} panel(es) solar(es) y un inversor ${q.inverterTier?.capacityLabel ?? "—"}, rieles y herrajes para la fijación de los paneles sobre techo, protecciones eléctricas AC y DC, así como todos los suministros necesarios para la instalación eléctrica y mecánica.\n\nIncluye instalación, configuración y puesta en marcha del sistema, monitoreo en línea de la producción fotovoltaica y del consumo de la propiedad (requiere conexión WiFi), además de la gestión de los trámites de inyección a la red ante la compañía distribuidora de energía eléctrica.`,
  },
  en: {
    ...
    description: (q) =>
      `Grid-tied Photovoltaic System (PV) designed for an average consumption of ${q.consumoMensualKwh} kWh per month. The system is made up of ${q.panelesNecesarios} solar panel(s) and a ${q.inverterTier?.capacityLabel ?? "—"} inverter, rails and mounting hardware to fix the panels to the roof, AC and DC electrical protections, and all supplies necessary for the electrical and mechanical installation.\n\nIncludes installation, configuration and commissioning of the system, online monitoring of the photovoltaic production and property consumption (requires WiFi connection), plus managing the grid-injection paperwork with the utility company.`,
  },
};
...
const LETTERHEAD_URL = "/cotizardor/quote-letterhead.png";
...
export async function generateQuotePdf(
  ...
  const letterhead = await loadImage(LETTERHEAD_URL);
  ...
  t.description(quote)
    .split("\n\n")
    .forEach((paragraph) => {
      const lines = doc.splitTextToSize(paragraph, productColWidth);
      doc.text(lines, productColX, descY);
      descY += lines.length * 11 + 11;
    });
